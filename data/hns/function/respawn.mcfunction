...
# If death -> seeker
execute if score hiderDeath hiddenInfo matches 1 if score Hiders: Info > #One hiddenInfo if score isGameRunning hiddenInfo matches 1 run title @s[team=!Seekers] title {"text":"You are a SEEKER!","color":"red"}
execute if score hiderDeath hiddenInfo matches 1 if score Hiders: Info > #One hiddenInfo if score isGameRunning hiddenInfo matches 1 run title @s[team=!Seekers] subtitle {"text":"Kill all hiders","color":"white"}
